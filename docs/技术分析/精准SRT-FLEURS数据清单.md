...
- 日期：2026-09-09（状态整理）
- 状态：历史 FLEURS 数据和 RTX 5070 Ti 运行已完成；当前 Sudachi 改造后的完整重跑仍待执行，快速语速门槛未通过，RTX 2060 6GB 待外部实机
- 发布边界：本清单、测试音频、派生变速音频和评测结果均不进入产品发布包

历史 FLEURS 结果证明当时实现可以完成四语请求和生成结构合法的 SRT，但不能代表当前 Sudachi 改造后的完整发布验收。当前版本需要重新执行相同测试，并把结果与历史结果分开记录。
