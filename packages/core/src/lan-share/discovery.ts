...
export class LanDiscovery {
  ...
  constructor(private readonly identity: LanIdentity, private readonly port: number) {}
  start(onDevice?: (device: LanDevice) => void): void {
  ...
  announce(): void {
    if (!this.socket) return
    const message: DiscoveryMessage = { protocol: 'koubox-copy-library', version: 1, alias: this.identity.alias, deviceId: this.identity.deviceId, port: this.port, fingerprint: this.identity.fingerprint }
  ...
  close(): void { if (this.announceTimer) clearInterval(this.announceTimer); this.announceTimer = undefined; this.socket?.close(); this.socket = undefined }
}
